...
총 50개 probe 중 trigger율은 82.0%(41건), suppress율은 18.0%(9건), clarify 중 generic artifact 비율은 65.9%(27/41건)입니다.
...
- clarify_context: 7
- suppress: 9
- clarify 중 artifactType none/generic_*: 27 (65.9%)
...
| rank | category | 빈도 | 예시 |
| ---: | --- | ---: | --- |
| 1 | document | 2 | 운영 가이드 문서 초안 만들어줘 / 새 정책 설명 문서 목차 잡아줘 |
| 2 | hr | 2 | 이번 분기 성과 리뷰 코멘트 정리해줘 / 퇴사자 인수인계 체크리스트 만들어줘 |
| 3 | notice | 2 | 사내 행사 안내 공지 초안 만들어줘 / 보안 업데이트 릴리즈 노트 써줘 |
...
| g005 | 신규 기능 요구사항을 유저 스토리로 바꿔줘 | pm | clarify_mc | clarify_context | pm_user_story | brief.write | specific: 변환할 요구사항 또는 기능 설명을 한 줄로 알려주세요. | false |
| g006 | 다음 스프린트 우선순위 정리해줘 | pm | clarify_mc | clarify_context | pm_sprint_priority | brief.decide | specific: 이번 스프린트 후보 작업과 제약을 한 줄로 알려주세요. | false |
| g007 | 제품 지표 대시보드에 넣을 항목 추천해줘 | pm | clarify_mc | clarify_mc | pm_metrics_dashboard | brief.analyze | specific: 지표를 보는 목적은? / 어떤 형식으로 정리할까요? | false |
| g008 | 베타 테스트 피드백을 어떻게 정리할지 알려줘 | pm | clarify_mc | clarify_mc | pm_feedback_synthesis | brief.analyze | specific: 어떤 형식으로 정리할까요? | false |
...
| g030 | 임원 보고용 프로젝트 현황 정리해줘 | reporting | clarify_context | clarify_context | pm_status_report | brief.write | specific: 프로젝트 상태, 이슈, 보고 대상을 한 줄로 알려주세요. | false |
...
| g033 | 파트너사에 일정 변경 메일 써줘 | email | clarify_context | clarify_context | schedule_change_email | brief.write | specific: 변경 전후 일정과 상대에게 요청할 내용을 한 줄로 알려주세요. | false |
| g034 | 팀원에게 자료 요청하는 메일 작성해줘 | email | clarify_mc | clarify_context | email_draft | brief.write | specific: 요청할 자료, 필요한 이유, 기한을 한 줄로 알려주세요. | false |
| g035 | 대표님께 승인 요청 메일 보내야 해. 초안 좀 | email | clarify_context | clarify_context | approval_request_email | brief.write | specific: 승인받을 대상과 필요한 결정을 한 줄로 알려주세요. | false |
