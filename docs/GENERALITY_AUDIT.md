...
생성 일시: 2026-05-29T04:14:26.590Z

이 문서는 측정 전용 리포트입니다. 제품 로직, core rule/compiler, runtime, governance 산출물은 변경하지 않습니다.

## 1. 한 줄 요약

총 50개 probe 중 trigger율은 68.0%(34건), suppress율은 32.0%(16건), clarify 중 generic artifact 비율은 82.4%(28/34건)입니다.
...
- clarify_context: 1
- suppress: 16
- clarify 중 artifactType none/generic_*: 28 (82.4%)
...
- g003 [hr] 이번 분기 성과 리뷰 코멘트 정리해줘 → expected=clarify_context, actual=suppress, artifact=none
- g008 [pm] 베타 테스트 피드백을 어떻게 정리할지 알려줘 → expected=clarify_mc, actual=suppress, artifact=none
- g013 [finance] 월별 손익 보고서 요약해줘 → expected=clarify_context, actual=suppress, artifact=none
- g017 [legal] 이 계약서 리스크 검토해줘 → expected=clarify_context, actual=suppress, artifact=none
- g018 [legal] 개인정보 처리방침 개정 안내문 써줘 → expected=clarify_context, actual=suppress, artifact=none
- g030 [reporting] 임원 보고용 프로젝트 현황 정리해줘 → expected=clarify_context, actual=suppress, artifact=none
- g039 [notice] 보안 업데이트 릴리즈 노트 써줘 → expected=clarify_mc, actual=suppress, artifact=none
...
| rank | category | 빈도 | 예시 |
| ---: | --- | ---: | --- |
| 1 | email | 3 | 파트너사에 일정 변경 메일 써줘 / 팀원에게 자료 요청하는 메일 작성해줘 / 대표님께 승인 요청 메일 보내야 해. 초안 좀 |
| 2 | pm | 3 | 신규 기능 요구사항을 유저 스토리로 바꿔줘 / 다음 스프린트 우선순위 정리해줘 / 제품 지표 대시보드에 넣을 항목 추천해줘 |
| 3 | document | 2 | 운영 가이드 문서 초안 만들어줘 / 새 정책 설명 문서 목차 잡아줘 |
| 4 | reporting | 2 | 주간 업무 보고서 초안 써줘 / KPI 리뷰 회의 자료 구조 잡아줘 |
| 5 | retrospective | 2 | 프로젝트 회고 보고서 구조 잡아줘 / 이번 실패에서 배운 점 정리해줘 |
...
| g003 | 이번 분기 성과 리뷰 코멘트 정리해줘 | hr | clarify_context | suppress | none | brief.extract | 질문 없음 | true |
...
| g008 | 베타 테스트 피드백을 어떻게 정리할지 알려줘 | pm | clarify_mc | suppress | none | brief.extract | 질문 없음 | true |
| g009 | API 에러 로그 분석 계획 세워줘 | dev | clarify_mc | clarify_mc | none | brief.analyze | generic: 무엇을 중심으로 분석하면 좋을까요? / 분석의 목적은? | true |
| g010 | 코드 리뷰 체크리스트 만들어줘 | dev | clarify_mc | clarify_mc | none | brief.generic | generic: 원하는 도움은? / 누가 읽거나 사용하게 되나요? | true |
...
| g013 | 월별 손익 보고서 요약해줘 | finance | clarify_context | suppress | none | brief.extract | 질문 없음 | true |
...
| g017 | 이 계약서 리스크 검토해줘 | legal | clarify_context | suppress | none | brief.generic | 질문 없음 | true |
| g018 | 개인정보 처리방침 개정 안내문 써줘 | legal | clarify_context | suppress | none | brief.generic | 질문 없음 | true |
| g019 | 서비스 이용약관 변경 체크리스트 만들어줘 | legal | clarify_mc | clarify_mc | none | brief.generic | generic: 원하는 도움은? / 누가 읽거나 사용하게 되나요? | true |
...
| g030 | 임원 보고용 프로젝트 현황 정리해줘 | reporting | clarify_context | suppress | none | brief.extract | 질문 없음 | true |
...
| g039 | 보안 업데이트 릴리즈 노트 써줘 | notice | clarify_mc | suppress | none | brief.generic | 질문 없음 | true |
...
| g045 | 운영 가이드 문서 초안 만들어줘 | document | clarify_mc | clarify_mc | none | brief.generic | generic: 원하는 도움은? / 답변 형식은? | true |
